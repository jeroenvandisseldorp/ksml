...
public class PrintOperationParser extends OperationParser<PrintOperationDefinition> {
    public PrintOperationParser(ParseContext context, TopologyResources resources) {
        super(context, KSMLDSL.Operations.PRINT, resources);
    }

    @Override
    public StructsParser<PrintOperationDefinition> parser() {
        final var contentParser = structsParser(
                PrintOperationDefinition.class,
                "",
                ...
                optional(functionField(KSMLDSL.Operations.Print.MAPPER, "A function to convert record into a string for output", new KeyValuePrinterDefinitionParser(context,false))),
                (name, filename, label, mapper, tags) -> new PrintOperationDefinition(operationConfig(name, tags), filename, label, mapper));
