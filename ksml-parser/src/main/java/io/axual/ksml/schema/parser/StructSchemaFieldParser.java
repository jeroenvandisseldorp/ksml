...
public class StructSchemaFieldParser extends BaseParser<StructSchema.Field> {
    ...
    public StructSchema.Field parse(ParseNode node) {
        ...
        final var schemaAndRequired = deduceSchemaAndRequired(schema, required);
     ...
     * @return A {@link SchemaAndRequired} record containing:
     * - The effective {@link DataSchema} (possibly adjusted if optional).
     * - The required status as a {@code boolean}.
     */
    private static SchemaAndRequired deduceSchemaAndRequired(DataSchema schema, Boolean required) {
        // If "required" is explicitly provided, use it directly.
        if (required != null) return new SchemaAndRequired(schema, required);

        // If schema is not a UnionSchema, assume the field is required.
        if (!(schema instanceof UnionSchema unionSchema)) return new SchemaAndRequired(schema, true);
        ...
        if (!unionSchema.contains(DataSchema.NULL_SCHEMA)) return new SchemaAndRequired(schema, true);
        ...
        return new SchemaAndRequired(new UnionSchema(newMembers.toArray(UnionSchema.Member[]::new)), false);
    }
}
