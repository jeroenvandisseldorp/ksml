...
public class JoinOperationParser extends StoreOperationParser<JoinOperation> {
    private final MultiSchemaParser<JoinOperation> parser;

    public JoinOperationParser(String namespace) {
        super(namespace, "join");
        final var joinStreamParser = structParser(
                ...
                (type, name, stream, valueJoiner, timeDifference, grace, store) -> new JoinOperation(storeOperationConfig(name, null, store), stream, valueJoiner, timeDifference, grace));
        ...
        final var joinTableParser = structParser(
                ...
                storeField(false, "Materialized view of the joined streams", null),
                (type, name, table, foreignKeyExtractor, valueJoiner, grace, partitioner, otherPartitioner, store) -> new JoinOperation(storeOperationConfig(name, null, store), table, foreignKeyExtractor, valueJoiner, grace, partitioner, otherPartitioner));
        ...
        final var joinGlobalTableParser = structParser(
                ...
                storeField(false, "Materialized view of the joined streams", null),
                (type, name, globalTable, mapper, valueJoiner, store) -> new JoinOperation(storeOperationConfig(name, null, store), globalTable, mapper, valueJoiner));

        final var map = new LinkedHashMap<String, Pair<String, MultiSchemaParser<? extends JoinOperation>>>();
