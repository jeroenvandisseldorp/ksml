...
public class ProducerDefinitionParser extends TopologyResourceAwareParser<ProducerDefinition> {
    public ProducerDefinitionParser(ParseContext context, TopologyResources resources) {
    ...
    public StructsParser<ProducerDefinition> parser() {
        return structsParser(
                ...
                functionField(Producers.GENERATOR, "The function that generates records", new GeneratorDefinitionParser(context, false)),
                optional(functionField(Producers.CONDITION, "A function that validates the generator's result message. Returns \"true\" when the message may be produced on the topic, \"false\" otherwise.", new PredicateDefinitionParser(context, false))),
                optional(functionField(Producers.UNTIL, "A predicate that returns true to indicate producing should stop.", new PredicateDefinitionParser(context, false))),
                topicField(Producers.TARGET, "The topic to produce to", new TopicDefinitionParser(context, resources(), false)),
