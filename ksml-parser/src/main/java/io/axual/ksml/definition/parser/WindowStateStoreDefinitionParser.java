...
public class WindowStateStoreDefinitionParser extends DefinitionParser<WindowStateStoreDefinition> implements NamedObjectParser {
    ...
    public WindowStateStoreDefinitionParser(ParseContext context, boolean requireStoreType, boolean requireKeyValueType) {
        super(context);
    ...
    protected StructsParser<WindowStateStoreDefinition> parser() {
        final var nameField = optional(stringField(KSMLDSL.Stores.NAME, false, "The name of the window store. If this field is not defined, then the name is derived from the context."));
