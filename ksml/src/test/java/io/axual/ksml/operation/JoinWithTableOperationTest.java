...
class JoinWithTableOperationTest extends OperationTestBase {
    ...
    void applyToStreamReturnsStream() {
        ...
        final var operation = new JoinWithTableOperation(
                storeConfig("join"), tableDefinition(), foreignKeyExtractor(), valueJoiner(), null, null, null);

        assertThat(operation.apply(input, mockContext())).isInstanceOf(KStreamWrapper.class);
        verify(stream).join(any(KTable.class), any(ValueJoinerWithKey.class), any(Joined.class));
    }
    ...
    void applyToTableReturnsTable() {
        ...
        verify(table).join(any(KTable.class), any(Function.class), any(ValueJoiner.class), any(TableJoined.class));
    }
    ...
    void applyToTableWithStoreAndPartitioners() {
        ...
        final var input = new KTableWrapper(table, key(), value());
        final var operation = new JoinWithTableOperation(
                storeConfig("join", keyValueStore("store")), tableDefinition(), foreignKeyExtractor(), valueJoiner(),
                null, streamPartitioner(), streamPartitioner());

        assertThat(operation.apply(input, mockContext())).isInstanceOf(KTableWrapper.class);
        // Foreign-key join with a materialized store and both partitioners.
        verify(table).join(any(KTable.class), any(Function.class), any(ValueJoiner.class), any(TableJoined.class), any(Materialized.class));
    }
    ...
    void applyToTableWithoutForeignKeyUsesPrimaryKeyJoin() {
        ...
        final var input = new KTableWrapper(table, key(), value());
        final var operation = new JoinWithTableOperation(
                storeConfig("join", keyValueStore("store")), tableDefinition(), null, valueJoiner(), null, null, null);

        assertThat(operation.apply(input, mockContext())).isInstanceOf(KTableWrapper.class);
        // Primary-key join uses the named + materialized overload.
        verify(table).join(any(KTable.class), any(ValueJoiner.class), any(Named.class), any(Materialized.class));
    }
}
