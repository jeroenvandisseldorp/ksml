...
public class UserTypeParser {
    ...
    public UserTypeParser(ParseContext context) {
        this.context = context;
    }

    public Parsed<UserType> parse(String type) {
        final var parsedTypes = parseListOfTypesAndNotation(type);
    ...
    private Parsed<UserType[]> parseListOfTypesAndNotation(String type) {
        if (type == null || type.isEmpty())
            return Parsed.ok(new UserType[]{new UserType(DataType.UNKNOWN)});

        type = type.trim();
        final var leftTerm = parseLeftMostTerm(type);
        if (leftTerm.isError()) return Parsed.error(leftTerm.errorMessage());
        final var parsedLeftTerm = parseTypeAndNotation(leftTerm.result());
        ...
        if (remainder.startsWith(TYPE_SEPARATOR)) {
            final var parsedRemainderTypes = parseListOfTypesAndNotation(remainder.substring(1));
    ...
    private record DecomposedType(String notation, String dataType) {
        public DecomposedType(String dataType) {
            this(null, dataType);
        }
    }

    private Parsed<UserType> parseTypeAndNotation(String composedType) {
        final var decomposedType = decompose(composedType);
        ...
        if (dataType.startsWith(SQUARE_BRACKET_OPEN)) {
            ...
            final var parsedValueType = parseTypeAndNotation(valueTypeStr);
        ...
        if (dataType.startsWith(UNION_TYPE + ROUND_BRACKET_OPEN)) {
            ...
            final var parsedMemberTypes = parseListOfTypesAndNotation(memberTypesStr);
            ...
            final var memberTypes = Arrays.stream(userTypesToDataTypes(parsedMemberTypes.result())).map(UnionType.Member::new).toArray(UnionType.Member[]::new);
        ...
        if (dataType.startsWith(WINDOWED_TYPE + ROUND_BRACKET_OPEN)) {
            ...
            final var parsedWindowType = parseTypeAndNotation(windowedTypeStr);
        ...
        if (dataType.startsWith(ROUND_BRACKET_OPEN)) {
            ...
            final var parsedElementTypes = parseListOfTypesAndNotation(elementTypesStr);
        ...
        // From here on, assume the notation is correct and exists

        // If the dataType (ie. schema) is empty, then return the notation with a default type
        if (dataType.isEmpty())
            return Parsed.ok(new UserType(notation, DataNull.DATATYPE));

        // Notation type with or without a specific schema
        final var notationName = notation != null ? notation : dataType;
        // If notation is given, then use the data type as is, else assume datatype represents the schemaless notation
        if (context.isNotation(notationName)) {
            if (notation != null) {
                // From here on, assume the dataType contains a schema name for the notation
                final var ntn = context.notation(notationName);
                // Load the schema
                final var schema = context.loadSchema(ntn, dataType, false);
                final var schemaType = new DataTypeDataSchemaMapper().fromDataSchema(schema);
                // If an assignable schema is loaded, then return okay, error otherwise
                return ntn.defaultType().isAssignableFrom(schemaType).isAssignable()
                        ? Parsed.ok(new UserType(notation, schemaType))
                        : Parsed.error("Notation does not allow for type: notation=" + ntn.name() + ", type=" + dataType);
            }
            // Schemaless notation, so return a schemaless struct type
            return Parsed.ok(new UserType(notationName, new StructType()));
        }
    ...
    private DecomposedType decompose(String composedType) {
        ...
        // Check if the user type contains a colon (usertype is of the form "notation:datatype") AND that this colon
        ...
        if (posColon < posOpenRound && posColon < posOpenSquare) {
            ...
            final var parsedDataType = composedType.substring(composedType.indexOf(NOTATION_SEPARATOR) + 1);

            // Return the parsed notation and datatype
            return new DecomposedType(parsedNotation, parsedDataType);
        }

        // Return the whole type string to be processed further, along with the default notation
        return new DecomposedType(composedType);
    }
