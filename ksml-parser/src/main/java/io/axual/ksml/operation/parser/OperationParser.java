...
@Getter
public abstract class OperationParser<T extends OperationDefinition> extends TopologyResourceAwareParser<T> implements NamedObjectParser {
    private String defaultShortName;
    private String defaultLongName;
    protected final String type;

    protected OperationParser(ParseContext context, String type, TopologyResources resources) {
    ...
    protected StructsParser<StateStoreDefinition> storeField(String childName, boolean required, String doc, StoreType expectedStoreType) {
        final var stateStoreParser = new StateStoreDefinitionParser(context, expectedStoreType, false);
        final var resourceParser = new TopologyResourceParser<>(context, "state store", childName, doc, (name, context) -> resources().stateStore(name), stateStoreParser);
