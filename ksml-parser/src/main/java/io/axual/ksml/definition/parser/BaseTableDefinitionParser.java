...
import io.axual.ksml.resource.TopologyBaseResources;
...
public abstract class BaseTableDefinitionParser<T> extends TopologyBaseResourceAwareParser<T> {
    ...
    public BaseTableDefinitionParser(ParseContext context, TopologyBaseResources resources, boolean isJoinTarget, String tableType) {
    ...
    protected StructsParser<FunctionDefinition> timestampExtractorField() {
        return optional(functionField(KSMLDSL.Streams.TIMESTAMP_EXTRACTOR, "A function that extracts the event time from a consumed record", new TimestampExtractorDefinitionParser(context, false)));
    }
    ...
    protected StructsParser<FunctionDefinition> partitionerField() {
        return optional(functionField(KSMLDSL.Streams.PARTITIONER, "A function that determines to which topic partition a given message needs to be written", new StreamPartitionerDefinitionParser(context, false)));
    }

    protected StructsParser<KeyValueStateStoreDefinition> storeField() {
        final var storeParser = new StateStoreDefinitionParser(context, StoreType.KEYVALUE_STORE, false);
        final var resourceParser = new TopologyResourceParser<>(context, "state store", KSMLDSL.Streams.STORE, "KeyValue state store definition", null, storeParser);
