...
public class TopicDefinitionParser extends TopologyBaseResourceAwareParser<TopicDefinition> {
    ...
    public TopicDefinitionParser(ParseContext context, TopologyBaseResources resources, boolean isSource) {
    ...
    public StructsParser<TopicDefinition> parser() {
        ...
        if (isSource) return structsParser(
                ...
                optional(functionField(Streams.TIMESTAMP_EXTRACTOR, "A function extracts the event time from a consumed record", new TimestampExtractorDefinitionParser(context, false))),
                optional(functionField(KSMLDSL.Streams.PARTITIONER, "A function that determines to which topic partition a given message needs to be written", new StreamPartitionerDefinitionParser(context, false))),
