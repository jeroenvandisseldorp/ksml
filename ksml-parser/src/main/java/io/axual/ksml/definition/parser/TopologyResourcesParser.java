...
public class TopologyResourcesParser extends DefinitionParser<TopologyResources> {
    ...
    public TopologyResourcesParser(ParseContext context, String namespace) {
        super(context);
        baseResourcesParser = new TopologyBaseResourcesParser(context, namespace);
    }

    @Override
    public StructsParser<TopologyResources> parser() {
        final var dummyResources = new TopologyBaseResources("dummy");
        final var streamsParser = optional(mapField(STREAMS, "stream", "stream definition", "Streams that can be referenced in producers and pipelines", new StreamDefinitionParser(context, dummyResources, false)));
        final var tablesParser = optional(mapField(TABLES, "table", "table definition", "Tables that can be referenced in producers and pipelines", new TableDefinitionParser(context, dummyResources, false)));
        final var globalTablesParser = optional(mapField(GLOBAL_TABLES, "globalTable", "globalTable definition", "GlobalTables that can be referenced in producers and pipelines", new GlobalTableDefinitionParser(context, dummyResources, false)));
