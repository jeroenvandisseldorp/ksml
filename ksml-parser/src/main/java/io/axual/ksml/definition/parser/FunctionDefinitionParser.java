...
public abstract class FunctionDefinitionParser<T extends FunctionDefinition> extends DefinitionParser<T> {
    ...
    protected FunctionDefinitionParser(ParseContext context, boolean requireType) {
        super(context);
    ...
    protected StructsParser<T> parserWithStores(Class<T> resultClass, String type, String description, Constructor1<T, FunctionDefinition> constructor) {
        return parser(resultClass, description, true, (name, params, globalCode, code, expression, resultType, stores, tags) -> FunctionDefinition.as(name, type, params, globalCode, code, expression, resultType, stores), constructor);
    }
    ...
    protected StructsParser<T> parserWithoutStores(Class<T> resultClass, String type, String description, Constructor1<T, FunctionDefinition> constructor) {
        return parser(resultClass, description, false, (name, params, globalCode, code, expression, resultType, stores, tags) -> FunctionDefinition.as(name, type, params, globalCode, code, expression, resultType, null), constructor);
    }
    ...
    private StructsParser<T> parser(Class<T> resultClass, String description, boolean includeStores, Constructor7<FunctionDefinition, String, List<ParameterDefinition>, String, String, String, UserType, List<String>> innerConstructor, Constructor1<T, FunctionDefinition> outerConstructor) {
        ...
        final var params = optional(listField(Functions.PARAMETERS, "parameter", "parameter", "A list of parameters to be passed into the " + description, new ParameterDefinitionParser(context)));
