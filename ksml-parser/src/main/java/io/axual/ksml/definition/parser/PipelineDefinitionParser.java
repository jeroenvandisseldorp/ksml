...
import io.axual.ksml.topology.PipelineDefinition;
import io.axual.ksml.dsl.KSMLDSL;
import io.axual.ksml.operation.AsOperationDefinition;
...
public class PipelineDefinitionParser extends TopologyResourceAwareParser<PipelineDefinition> implements NamedObjectParser {
    ...
    protected PipelineDefinitionParser(ParseContext context, TopologyResources resources) {
        this(context, resources, true);
    }

    protected PipelineDefinitionParser(ParseContext context, TopologyResources resources, boolean parseSource) {
        super(context, resources);
        this.parseSource = parseSource;
        asParser = new AsOperationParser(context, resources());
        branchParser = new BranchOperationParser(context, resources(), parseSource);
        forEachParser = new ForEachOperationParser(context, resources());
        printParser = new PrintOperationParser(context, resources());
        toTopic = new ToOperationParser(context, resources());
        toTne = new ToTopicNameExtractorOperationParser(context, resources());
    }

    @Override
    public StructsParser<PipelineDefinition> parser() {
        final var sourceField = topologyResourceField("source", KSMLDSL.Pipelines.FROM, "Pipeline source", (name, tags) -> resources().topic(name), new TopicDefinitionParser(context, resources(), true));
        ...
        return structsParser(
                ...
                optional(listField(KSMLDSL.Pipelines.VIA, "step", "step", "A series of operations performed on the input stream", new PipelineOperationParser(context, resources()))),
                optional(asParser),
                ...
                (name, from, via, as, branch, forEach, print, toTopic, toTne, tags) -> {
                    ...
                    // as identifier.
                    var sinkOperation = shortName != null ? new AsOperationDefinition(new OperationConfig(resources().getUniqueOperationName(longName), tags), shortName) : null;
